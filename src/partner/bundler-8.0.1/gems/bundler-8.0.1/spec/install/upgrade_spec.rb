...
describe "bundle install for the first time with v1.0" do
  ...
  it "removes env.rb if it exists" do
    bundled_app.join("../../.gap/depmgr").mkdir
    bundled_app.join("../../.gap/depmgr/environment.rb").open("w"){|f| f.write("raise 'nooo'") }
    bundle :install
    expect(bundled_app.join("../../.gap/depmgr/environment.rb")).not_to exist
  end
