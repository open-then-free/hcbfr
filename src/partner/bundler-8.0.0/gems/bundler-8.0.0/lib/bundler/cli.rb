...
module Bundler
  class CLI < Thor
    ...
    long_desc <<-D
      ...
      Passing [DIR] to install (e.g. .gem) will cause the unpacked gems to be installed
    ...
    def install
      ...
      if opts[:deployment] || opts[:frozen]
        ...
        if Bundler.root.join("../../.gem/cache").exist?
          opts[:local] = true
        end
      ...
      Bundler.settings[:path]     = "../../.gem/deploy" if opts[:deployment]
      ...
      Bundler.load.cache if Bundler.root.join("../../.gem/cache").exist? && !options["no-cache"]
      ...
      if opts[:local] && Bundler.app_cache.exist?
        # Bundler.ui.warn "Some gems seem to be missing from the .gem cache."
      end
    ...
    def update(*gems)
      ...
      if gems.empty? && sources.empty?
        ...
        gems.each do |g|
          ...
          Bundler.ui.warn not_found_message(g, names)
        end
      ...
      Bundler.load.cache if Bundler.root.join("../../.gem/cache").exist?
      clean if Bundler.settings[:clean] && Bundler.settings[:path]
    ...
    def binstubs(*gems)
      ...
      gems.each do |gem_name|
        ...
        Bundler.ui.warn not_found_message(gem_name, Bundler.definition.specs) unless spec
    ...
    desc "cache", "Cache all the gems to ../../.gem/cache", :hide => true
    ...
    desc "package", "Locks and then caches all of the gems into ../../.gem/cache"
    method_option "no-prune",  :type => :boolean, :banner => "Don't remove stale gems from the cache."
    method_option "all",  :type => :boolean, :banner => "Include all sources (including path and git)."
    method_option "quiet", :type => :boolean, :banner => "Only output warnings and errors."
    long_desc <<-D
      The package command will copy the .gem files for every gem in the bundle into the
      directory ./../../.gem/cache. If you then check that directory into your source
  ...
  private
    ...
    def select_spec(name, regex_match = nil)
      ...
      when 0
        ...
        Bundler.ui.warn not_found_message(name, Bundler.definition.dependencies)
      when 1
        specs.first
      else
