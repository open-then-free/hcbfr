module Spec
  module Helpers
    ...
    def config(config = nil)
      path = bundled_app('../../.gap/depmgr/config.file')
    ...
    def cache_gems(*gems)
      ...
      FileUtils.rm_rf("#{bundled_app}/../../.gem/cache")
      FileUtils.mkdir_p("#{bundled_app}/../../.gem/cache")

      gems.each do |g|
        path = "#{gem_repo1}/gems/#{g}.gem"
        raise "OMG `#{path}` does not exist!" unless File.exist?(path)
        FileUtils.cp(path, "#{bundled_app}/../../.gem/cache")
      end
    end
    ...
    def simulate_new_machine
      ...
      FileUtils.rm_rf bundled_app('../../.gap/depmgr')
    end
