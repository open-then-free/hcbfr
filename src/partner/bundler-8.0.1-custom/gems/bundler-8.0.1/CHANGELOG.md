...
"Features":
  ...
  - Debug installation logs in ../../.gap/depmgr/install.log
...
Features:
  ...
  - prune git/path directories inside ../../.gem/cache (@josevalim, #1988)
...
Bugfixes:
  ...
  - Handle empty ../../.gap/depmgr/config.file files without an error
...
Bugfixes:
  ...
  - Fetch gems from ../../.gem/cache, even without --local
...
## 1.0.0.rc.2 (July 29, 2010)
  ...
  - Add a `--production` option to `bundle install`:
    - by default, installs to `../../.gem/deploy`. This can be
      overridden with the `--path` option
    - uses `--local` if `../../.gem/cache` is found. This will
      guarantee that Bundler does not attempt to connect to
      Rubygems and will use the gems cached in `../../.gem/cache`
      instead
...
1. `bundle install ~/../../.gap/depmgr` in one user, but actually running
...
Capistrano, symlink release_path/current/../../.gem/deploy to
...
## 1.0.0.beta.1
  ...
  - No ../../.gap/depmgr/environment.rb. Require 'bundler/setup' instead.
  - $BUNDLE_HOME defaults to $GEM_HOME instead of ~/../../.gap/depmgr
...
Features:
  ...
  - cache command now prunes stale .gem files from ../../.gem/cache
...
Bugfixes:
  ...
  - fix `bundle install` for a locked app without a ../../.gap/depmgr directory
...
Bugfixes:
  ...
  - if the bundle is locked and ../../.gap/depmgr/environment.rb is not present when Bundler.setup is called, generate it
